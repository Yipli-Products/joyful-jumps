...
{	
	...
	/// </summary>
	public class GSGMultipleObjectPoolerObject
	{
	...
	/// </summary>
	public enum GSGPoolingMethods { OriginalOrder, OriginalOrderSequential, RandomBetweenObjects, RandomPoolSizeBased }

	/// <summary>
	/// This class allows you to have a pool of various objects to pool from.
	/// </summary>
	public class GSGMultipleObjectPooler : GSGObjectPooler
	{
		/// the list of objects to pool
		public List<GSGMultipleObjectPoolerObject> Pool;
		[Tooltip("A MultipleObjectPooler is a reserve of objects, to be used by a Spawner. When asked, it will return an object from the pool (ideally an inactive one) chosen based on the pooling method you've chosen.\n- OriginalOrder will spawn objects in the order you've set them in the inspector (from top to bottom)\n- OriginalOrderSequential will do the same, but will empty each pool before moving to the next object\n- RandomBetweenObjects will pick one object from the pool, at random, but ignoring its pool size, each object has equal chances to get picked\n- PoolSizeBased randomly choses one object from the pool, based on its pool size probability (the larger the pool size, the higher the chances it'll get picked)'...")]
		/// the chosen pooling method
		public GSGPoolingMethods PoolingMethod = GSGPoolingMethods.RandomPoolSizeBased;
		[Tooltip("If you set CanPoolSameObjectTwice to false, the Pooler will try to prevent the same object from being pooled twice to avoid repetition. This will only affect random pooling methods, not ordered pooling.")]
		...
		protected List<GSGMultipleObjectPoolerObject> _randomizedPool;
		...
		public override void FillObjectPool()
		{
			...
			_randomizedPool = new List<GSGMultipleObjectPoolerObject>() ;
			...
			{
				case GSGPoolingMethods.OriginalOrder:
					...
					break;
				case GSGPoolingMethods.OriginalOrderSequential:

					_pooledGameObjectsOriginalOrder = new List<GameObject>();

					// we store our poolsizes in a temp array so it doesn't impact the inspector
					foreach (GSGMultipleObjectPoolerObject pooledGameObject in Pool)
					{
			...
			if ((PoolingMethod==GSGPoolingMethods.OriginalOrder) || (PoolingMethod == GSGPoolingMethods.OriginalOrderSequential))
			{
		...
		public override GameObject GetPooledGameObject()
		{
			...
			switch (PoolingMethod)
			{
				case GSGPoolingMethods.OriginalOrder:
					...
					break;
				case GSGPoolingMethods.RandomPoolSizeBased:
					pooledGameObject =  GetPooledGameObjectPoolSizeBased();
					break;
				case GSGPoolingMethods.RandomBetweenObjects:
		...
		protected virtual GameObject GetPooledGameObjectOriginalOrder()
		{
			...
			GSGMultipleObjectPoolerObject searchedObject = GetPoolObject(_pooledGameObjects[_currentIndex].gameObject);
		...
		protected virtual GameObject GetPooledGameObjectPoolSizeBased()
		{
			...
			{	
				...
				else
				{
					...
					GSGMultipleObjectPoolerObject searchedObject = GetPoolObject(_pooledGameObjects[randomIndex].gameObject);
		...
		protected virtual GSGMultipleObjectPoolerObject GetPoolObject(GameObject testedObject)
		{
			...
			int i=0;
			foreach(GSGMultipleObjectPoolerObject poolerObject in Pool)
			{
		...
		{
			GSGMultipleObjectPoolerObject searchedObject = GetPoolObject(testedObject);
			if (searchedObject != null)
			{
