...
{
	public struct GSGStateChangeEvent<T> where T: struct, IComparable, IConvertible, IFormattable
	{
		public GameObject Target;
		public GSGStateMachine<T> TargetStateMachine;
		public T NewState;
		public T PreviousState;

		public GSGStateChangeEvent(GSGStateMachine<T> stateMachine)
		{
	...
	/// </summary>
	public interface GSGIStateMachine
	{
	...
	/// </summary>
	public class GSGStateMachine<T> : GSGIStateMachine where T : struct, IComparable, IConvertible, IFormattable
	{
		...
		/// To listen to these events, from any class, in its Start() method (or wherever you prefer), use GSGEventManager.StartListening(gameObject.GetInstanceID().ToString()+"XXXEnter",OnXXXEnter);
		...
		/// GSGEventManager.StartListening(gameObject.GetInstanceID().ToString()+"CrouchingEarlyUpdate",OnCrouchingEarlyUpdate); for example will listen to the Early Update event of the Crouching state, and 
		...
		public GSGStateMachine(GameObject target, bool triggerEvents)
		{
			this.Target = target;
		...
		public virtual void ChangeState(T newState)
		{
			...
			if (TriggerEvents)
			{
				GSGEventManager.TriggerEvent (new GSGStateChangeEvent<T> (this));
			}
		}
